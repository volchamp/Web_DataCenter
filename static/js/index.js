...
$(function() {
    ...
    function getData(menuCode, arry) {
        var childArry = getParentArry(menuCode, arry);
        if (childArry.length > 0) {
            for (var i in childArry) {
                if(childArry[i].parentCode=='0'){
                    if(i==0){
                        menus +='<li class="active">';
                    }else{
                        menus +='<li>';
                    }
                    menus +='<a href="#"><i class="'+childArry[i].menuIcon+'" style="margin-right: 5px;"></i>' +
                        '<span class="nav-label">'+childArry[i].menuName+'</span>' +
                        '<span class="fa arrow"></span>' +
                        '</a>';
                    menus +='<ul class="nav nav-second-level">';
                    getData(childArry[i].menuCode, arry);
                    menus +='</ul>';
                    menus +='</li>';
                }else{
                    menus += '<li>';
                    menus +='<a class="menuItem" href="javascript:void(0);" data-url="'+childArry[i].menuUrl+'"><i class="'+childArry[i].menuIcon+'" style="margin-right: 5px;"></i>'+childArry[i].menuName+'</a>';
                    getData(childArry[i].menuCode, arry);
                    menus +='</li>';
                }
            }
    ...
    function getParentArry(menuCode, arry){
        var newArry = new Array();
        for (var i in arry) {
            if (arry[i].parentCode == menuCode)
                newArry.push(arry[i]);
        }
        return newArry;
    }
    ...
    function getUserMenu() {
        XEAjax.get(`${curl}/user/getUserMenu`).then(data => {
            if(data.data){
                menus += '<li class="list-divider"></li>';
                getData('0', data.data);
                menus += '<li class="list-divider"></li>';
                //console.info(menus);
                //$("#mainnav-menu").empty();
    ...
    $.contextMenu({
        ...
        items: {
            ...
            /*"full": {
                ...
                callback: function(key, opt) {
                	setActiveTab(this);
                    var target = $('.app_iframe[data-id="' + this.data('id') + '"]');
                	target.fullScreen(true);
                }
            },*/
            "refresh": {
